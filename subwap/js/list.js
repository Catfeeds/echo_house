    ...
    $.get('/api/plot/info?id='+hid+'&phone='+phone, function(data) {
            ...
            if(detail.phones.length > 0) {
                for (var i = 0; i < detail.phones.length; i++) {
                    ...
                    // debugger;
                    $('.telephone-consult ul').append('<li><a onclick="makeCall(this)"  data-href="tel:'+detail.phonesnum[i]+'"><div class="telephone-place"><img class="consult-user-img" src="./img/'+icon+'"><div class="consult-text">'+detail.phones[i]+word+'</div></a>'+(detail.qfuidsarr[i]!=""?'<div onclick="setChat(this)" data-text="'+detail.qfuidsarr[i]+'" class="copy-weixin">在线聊天</div>':'')+'<a onclick="makeCall(this)"  data-href="tel:'+detail.phonesnum[i]+'"><img class="consult-tel-img" src="./img/tel-green.png"></div></a><div class="line"></div></li>');
                }
            }
...
function makeCall(obj) {
    $.get('/api/plot/callPhone?hid='+hid+'&key='+$(obj).data('href')+'&fxphone='+nowphone,function(data) {
        if(data.status=='success') {
