...
class UserExt extends User{
    ...
    public function getCanSubNum()
    {
        if($this->vip_expire>time()) {
            ...
            $num2 = Yii::app()->db->createCommand('select count(m.id) from plot_makert_user m left join plot p on m.hid=p.id where m.status=1 and m.expire>'.time().' and m.uid='.$this->id.' and p.status=1 and p.uid!='.$this->uid)->queryScalar();
