...
class PlotExt extends Plot{
    ...
    {
        if($owner = $this->owner) {
            $owner->qf_uid && Yii::app()->controller->sendNotice('恭喜您，'.$this->title.'已通过审核并已上线，若想成为对接人或进行续费。请点击'.Yii::app()->request->getHostInfo().'/subwap/duijieren.html?hid='.$this->id.' 进行付费，完成后您的联系方式将会自动显示到联系人电话列表首位。
